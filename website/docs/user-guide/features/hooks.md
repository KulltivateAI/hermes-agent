...
- Callback exceptions are logged and later callbacks continue. They are otherwise skipped, except on `pre_gateway_dispatch`, where each exception synthesizes a `skip` denial so a broken admission callback fails closed.
- If a Python plugin callback on a **timeout-bounded** hook (hot-path observers such as `post_tool_call` / `pre_llm_call`, plus the policy hooks `pre_tool_call` and `pre_gateway_dispatch`) **blocks** longer than `plugins.hook_callback_timeout` (default 30s, max 600), it is abandoned without joining the worker so the agent loop continues. Setting the timeout to `0` disables bounding for other hooks, but `pre_gateway_dispatch` keeps the default bound because gateway admission may not hang indefinitely. Timed-out or still-running policy callbacks **fail closed** (`pre_tool_call` blocks the tool; `pre_gateway_dispatch` denies the event); other bounded hooks fail open (skip). Hooks with a documented caller-thread contract (`subagent_stop`) are never moved onto a timeout worker. Shell hooks keep their own per-entry `timeout`.
...
| `pre_gateway_dispatch` | Directive/control | Incoming non-internal message before auth/pairing/dispatch; any `skip` denies the event, otherwise the first recognized `rewrite`, `allow`, or valid `authorize` controls flow. | `event`, `gateway`, `session_store` | Extremely privileged in-process objects expose inbound user/routing data and host handles; `authorize` bypasses normal sender auth for one event. |
...
**Return value:** `None` or a dict. Any `skip` result wins so a denial or synthesized callback-failure denial cannot be overridden by another callback's `authorize`; otherwise the first recognized action dict wins. Exceptions in plugin callbacks are caught and logged, then deny the event; a failure of the hook invocation itself also denies the event. Traffic admitted by Discord's `hook_mentions` adapter mode remains denied unless a callback returns a valid explicit `authorize` directive, so a missing, failing, or timed-out admission plugin fails closed.
...
| `{"action": "authorize", "text": "validated request", "clear_channel_context": true}` | Authorize **this event only**, optionally replace its text, and optionally discard normalized channel-history backfill before prompt preparation. `text`, when present, must be a string; `clear_channel_context`, when present, must be a boolean. A malformed directive does not authorize. Attachments are not cleared. |
| `{"action": "allow"}` / `None` | Normal dispatch — runs the full auth / pairing / agent-loop chain. |

`authorize` is a high-trust capability: enabling a plugin that returns it lets that plugin bypass normal gateway sender authorization for individual events. The receipt is process-local to the current `MessageEvent`; it does not mutate the sender, persist in session state, or authorize later messages. Admission plugins should validate `event.raw_message` and reject unexpected attachments before authorizing. Use `clear_channel_context: true` for machine requests so Discord history backfill cannot enter the admitted prompt.
