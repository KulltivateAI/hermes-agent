...
| `pre_gateway_dispatch` | Directive/control | Incoming non-internal message before auth/pairing/dispatch; first recognized `skip`, `rewrite`, `allow`, or valid `authorize` controls flow. | `event`, `gateway`, `session_store` | Extremely privileged in-process objects expose inbound user/routing data and host handles; `authorize` bypasses normal sender auth for one event. |
...
**Return value:** `None` or a dict. The first recognized action dict wins; remaining plugin results are ignored. Exceptions in plugin callbacks are caught and logged; ordinary traffic falls through to normal dispatch on error. Traffic admitted by Discord's `hook_mentions` adapter mode remains denied unless a callback returns a valid explicit `authorize` directive, so a missing, failing, or timed-out admission plugin fails closed.
